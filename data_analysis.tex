...
We assume wlog that $E[X] = E[Y] = 0$ 

\ul{Objective}: Given multivariate random vectors $X \in \mathbb{R}^{n}, Y \in \mathbb{R}^{m}$, find \textit{linear} mappings, $a \in \mathbb{R}^{n}, b \in \mathbb{R}^{m}$, also known as the \textbf{canonical directions}, such that for the random variables $U = a^{T}X$ and $V = b^{T}Y$, we have:
\begin{align*}
...
\[
  \max_{a,b} a^{T} \Sigma_{XY} b \quad \text{ s.t. } a^{T} \Sigma_{XX}a = 1, b^{T} \Sigma_{YY} b = 1
\]
...
We solve using Lagrange Multipliers, defining:
\[
