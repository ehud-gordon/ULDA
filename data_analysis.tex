...
\[
S \stackrel{\text{ def }}{=} \frac{1}{n-1} X^T X = \frac{1}{n-1} \sum_{i=1}^{n} \bm{x}^{(i)} (\bm{x}^{(i)})^{T} = \frac{1}{n-1} \sum_{i=1}^{n} \bm{x}^{(i)} \otimes  \bm{x}^{(i)}
\]
...
We perform PCA by computing $S = \frac{1}{n-1} X^{T} X = V \Lambda V^{T}$. 
...
If we denote $\mathcal{F}$ dimension by $m$, then $S_{\phi} \in \mathbb{R}^{m \times  m}$. 
This suggest a new problem - if $m >> n$, then computations become expensive. Luckily, we notice that:
...
\]
  Therefore, $S_{\phi} \bm{w}$ is a linear combination of $\phi(\bm{x}^{(i)})$, with the coefficients being  $\langle \phi(\bm{x}^{(i)}, \bm{w} \rangle$. 
Therefore, we can express $S_{\phi} \bm{w}$ as a linear combination of $\left\{ \phi(\bm{x}^{(i)}) \right\}_{i=1}^{n}$. 
...
Where $\alpha_j \in \mathbb{R}$ are the coefficients we wish to determine, and we write $\bm{\alpha}=[\alpha_{1}, \ldots, \alpha_n]^T$.  Plugging this into~\cref{eq:kernelEigenvectorDef}, we get:
\begin{align*}
...
Then we get that:
\begin{align*}
  S_{\phi} \bm{v} = \frac{1}{n} \sum_{i=1}^{n} \phi (\bm{x}^{(i)}) \sum_{j=1}^{n} \alpha_{j} K_{ij} 
\end{align*}

Define a new variable:
\[
  \beta_{i} \equiv \sum_{j=1}^{n} \alpha_j K_{ij}
\]
Then:
\[
  S_{\phi} \bm{v} = \frac{1}{n} \sum_{i=1}^{n} \phi(\bm{x}^{(i)}) \beta_i
\]
...
\[
  \frac{1}{n} \sum_{i=1}^{n} \phi(\bm{x}^{(i)}) \beta_i = \lambda \sum_{i=1}^{n} \alpha_i \phi(\bm{x}^{(i)})
\]
...
\begin{align*}
  & \frac{1}{n} \beta_i = \lambda \alpha_i \\
  \Rightarrow \ & \sum_{j=1}^{n} K_{ij} \alpha_j = n \lambda \alpha_i \\
  \Rightarrow \ & \big( K \bm{\alpha} \big)_i = n \lambda \alpha_i \\
  \Rightarrow \ & K \bm{\alpha} = n \lambda \bm{\alpha}
\end{align*}
Therefore, the original problem of finding eigenvectors for the $m \times m$ matrix $S_{\phi}$ is actually equivalent to finding the eigenvectors of the $n \times n$ matrix  $K$. Since $m >> n$, the second task is computationally easier. Using $k$, we compute matrix $K$, find its eigenvalues $\bm{\alpha}_1, \ldots \bm{\alpha}_{m}$ with eigenvalues $\lambda_1, \ldots, \lambda_{m}$, and proceed. 

Note that $K$ is defined by the inner product $K_{ij} = \langle \phi(\bm{x}^{(i)}), \phi(\bm{x}^{(j)}) \rangle$. Hence, in situations where such a computation is easy, i.e. in situations where we have a \textbf{kernel function} $k(\bm{x}^{(i)},\bm{x}^{(j)})$ such that:
\[
...
We finish by performing kernel PCA, projecting the lifted samples $\phi(\bm{x}^{(i)})$ onto the $m$-th largest eigenvector of the (centered) kernel matrix $K$, using \textbf{only the kernel function}:
\[
  k(\bm{x}^{(i)}, \bm{x}^{(j)}) = \langle \phi(\bm{x}^{(i)}, \phi(\bm{x}^{(j)}) \rangle
\]
Let $\bm{\alpha}_{m} = [(\bm{\alpha}_{m})_1, \ldots, (\bm{\alpha}_{m})_n]$ denote the m-th largest eigenvector of the centered kernel matrix $K$, and $\bm{v}_{m}$ the m-th largest eigenvector of $S_{\phi}$. 
Remember that $\bm{v}_{m} = \sum_{j=1}^{n} \phi(\bm{x}^{(j)}) \, (\bm{\alpha}_m)_j$. 
Then we have that:
\begin{align*}
  \langle \, \phi(\bm{x}^{(i)}) \,, \, \bm{v}_{m} \, \rangle &= \langle \, \phi(\bm{x}^{(i)}) \,, \, \sum_{j=1}^{n} \phi(\bm{x}^{(j)}) (\bm{\alpha}_m)_j \, \rangle  \\
  &=  \sum_{j=1}^{n} (\bm{\alpha}_m)_j \, \langle \, \phi(\bm{x}^{(i)}) \,, \, \phi(\bm{x}^{(j)}) \, \rangle \\
  &= \sum_{j=1}^{n} (\bm{\alpha}_m)_j \, k(i,j)
\end{align*}

And therefore, we can project the samples, and hence perform kernel PCA, using \textbf{only} the kernel function $k(i,j)$. 
