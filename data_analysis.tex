...
\begin{definition}[Probabilistic Model]  Assume the observed variable $\bm{x}$ is a random sample from an unknown underlying process, whose true probability distribution is $p_{\theta}(\bm{x})$. pth
\end{definition}

\begin{definition}[Generative Latent-Variable Model] A parametric family of joint probability distributions over observed variables $x$ and unobserved latent variables $z$, defined by:
  \[
    p_{\theta}(x,z) = p_{\theta}(z) p_{\theta}(x | z)
  \]
 where $\theta$ are the model parameters. The latent variables  
